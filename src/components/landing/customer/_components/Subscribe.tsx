import delivery from "@/assets/images/delivery.png";
import { Button } from "@/components/ui/button";
const Subscribe = () => {
  return (
    <div className="bg-[#D5F6E6] text-[#013929] p-5 md:p-10 flex flex-col items-center">
      <div className="md:flex items-center w-full md:w-[80%] justify-between">
        <div className="md:w-1/2 md:py-20 py-10 md:max-w-[40%]">
          <h1 className="text-4xl md:text-3xl mb-4 text-hero">
            Subscribe for weekly or monthly delivery
          </h1>
          <p className="my-2">
            You can choose to subscribe to subscribe to our weekly or monthly
            food ordering and delivering. Just subscribe and focus on other
            important stuffs. We gat you.
          </p>
          <Button
            disabled
            className="bg-[#EFFFFB] border border-secondary py-2 w-full md:w-[80%] rounded-full text-sm text-secondary my-4 md:my-2"
          >
            Coming soon
          </Button>
        </div>
        <div className="md:w-1/3 md:mt-0 mt-7 mb-[40px] md:mb-0">
          <img
            src={delivery}
            className="md:w-[25rem] w-[23rem]"
            alt="delivery"
          />
        </div>
      </div>
    </div>
  );
};

export default Subscribe;
